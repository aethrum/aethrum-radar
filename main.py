...
def generar_mensaje_emocional(dominante, scores, text, url=None):
    ...
    estado = "✅ Noticia Aprobada" if relevancia > 0 else "❌ Noticia Rechazada"
    otras = "\n".join([f"- {e}: {v}%" for e, v in ordenadas if e != dominante])
    fragmento = text.strip()[:500].replace("\n", " ")
...
def recibir_webhook():
    try:
        ...
        if texto == "/resumen":
            ...
            with open("registros.csv", newline='', encoding="utf-8") as f:
        ...
        return jsonify({"status": "ok", "emotion": emocion})
